...
const MyPillars = (props) => {
    ...
    const getData = (type) => {
        setLoader(true)
        axios.get(`${Constants.BASE_URL}business/request-business-collobration/${props?.route?.params?.userDetails?.business?.business_id}`)
            .then((response) => {
                if (response.data.response == 200) {
                    // console.log("response data",response.data.data.influencer.filter((i)=>i.collabration_status=='ended'));
                    if (type == 'ongoing') {
                        setTabs('ongoing')
                        setpillarsData(response.data.data.influencer.filter((i) => i.collabration_status == 'approve'));
                        setpillarsEndedData([])
                    }
                    else if (type == 'ended') {
                        setTabs('ended')
                        setpillarsData([])
                        setpillarsEndedData(response.data.data.influencer.filter((i) => i.collabration_status == 'ended'));
                    }
                    else if (type == 'pending') {
                        setTabs('pending')
                        setpillarsData([])
                        setpillarsEndedData(response.data.data.influencer.filter((i) => i.collabration_status == 'pending'));
                    }
                    else {
                        setTabs('rejected')
                        setpillarsData([])
                        setpillarsEndedData(response.data.data.influencer.filter((i) => i.collabration_status == 'reject'));
                    }
                    setLoader(false)
                }
                else {
